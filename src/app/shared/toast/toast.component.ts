  ...
  template: `
    <div class="fixed top-4 right-4 z-[9999] space-y-2 max-w-sm">
      @for (toast of toasts(); track toast.id) {
        <div 
          class="animate-slide-in-right rounded-xl shadow-2xl backdrop-blur-xl border overflow-hidden"
          [ngClass]="{
            'bg-green-500/90 border-green-400': toast.type === 'success',
            'bg-red-500/90 border-red-400': toast.type === 'error',
            'bg-yellow-500/90 border-yellow-400': toast.type === 'warning',
          ...
          <div class="p-4 flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              <p class="font-bold text-white text-sm">{{ toast.title }}</p>
              <p class="text-white/90 text-sm mt-1">{{ toast.message }}</p>
            </div>
